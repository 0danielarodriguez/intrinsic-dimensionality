...
corrida_normal <- function(Nrep=10,nsamp=100,p=10,sigma=sqrt(1), 
                    signal=c(10,8,6,4,2))
...
{    
  ...
  for(a in 1:Nrep)
  {
   ...
   # d_ef_nuestra_convRobMM <- dim_est_cross_grupos_rob(xx, tipo="covRobMM")
  #  d_ef_nuestra_covRobRocke <- dim_est_cross_grupos_rob(xx, tipo="covRobRocke")
    # d_ef_nuestra_fastmve <- dim_est_cross_grupos_rob(xx, tipo="fastmve")
    # d_ef_nuestra_initPP<- dim_est_cross_grupos_rob(xx, tipo="initPP")
    ...
    sigma <- ss_sigmas[s] #aca esta el ruido
      ...
      for(i in 3:length(enes))
      {
        ...
        nsamp <- enes[i]
        {
          ...
          corrida_normal(Nrep,nsamp=nsamp,p=10,sigma=sigma, 
                                     signal=signal)
    ...
    for(s in 1:3)  
  {
    ...
    for(i in 1:length(enes))
    {
      nsamp <- enes[i]
      {
        ...
        #        ARCHIVORESULTADOS<-paste("Simu_n_",nsamp, "_Nrep_", Nrep,"signal", signal[1], "_SM=",student_multivariada,".txt",sep="")  
...
colnames(pirulo_bishop)<-c( "signal", "nsamp",  "npPCA",
                           "robPCA", 
                           "pPCA",
                           "L","Bayes")
