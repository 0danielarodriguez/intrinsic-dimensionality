...
#setwd("C:/Users/drodriguez/Dropbox/daniela/trabajos/mariela-dani/otra-vez-autovalores/statistical papers/Revision/simus_articulo_lili/TABLE 3")
#setwd("~/Dropbox/mariela-dani/otra-vez-autovalores/simus_articulo/p_10__normal_nrep_1000")
setwd("~/Dropbox/mariela-dani/otra-vez-autovalores/statistical papers/Revision/simus_articulo_lili/TABLE 3")
getwd()
...
corrida_normalmix <- function(Nrep=10,nsamp=100,p=10,sigma=sqrt(1), signal=c(10,8,6,4,2),
                              epsilon=0.1, K=10, gamma=0)
...
{    
    ...
    #Matriz verdadera
    Sigma= W%*%t(W)+sigma^2*diag(p)
    Sigma_um <- diag(sqrt(svd(Sigma)$d)) #rais cuadrada de la matriz
    Sigma_mum <- diag(sqrt(svd(Sigma)$d)^{-1}) #para estandarizar: Sigma menos un medio 
...
set.seed(1200)
  for(a in 1:Nrep)
  {
    ...
    # Contamino
    m=floor(epsilon*nsamp)
    
    xc <- xx[1:m,]  #  elijo datos a contaminar (los primeros)
   
    xc <- xc%*%Sigma_mum #estadarizo 
    xc[,1] <- gamma*xc[,1]+K    #contamino 
    xc <- xc%*%Sigma_um #devuelvo a escala original
    
    xx[1:m,] <- xc #junto con los datos buenos
