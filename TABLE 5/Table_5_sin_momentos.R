...
setwd("~/Dropbox/mariela-dani/otra-vez-autovalores/statistical papers/Revision/simus_articulo_lili/TABLE 5")
getwd()
...
corrida_normalmix <- function(Nrep=10,nsamp=100,p=10,sigma=sqrt(1), signal=c(10,8,6,4,2),df=1)
#epsilon= proporcion de contaminacion
#contaminacion: x_1<-gamma x_1+K  (pag 234 Yohai)   
  #epsilon = 0.1, 0.2
  #gamma=0, 0.5
  #K=seq(1,12)
...
set.seed(1200)
  ...
  #  d_ef_nuestra=0
   ...
   d_ef_nuestra_covRob <- tryCatch({
     dim_est_cross_grupos_rob(xx, tipo="covRob")
   }, error = function(e) {
     "error" # Devuelve "error" si hay un fallo
   })
...
#    d_ef_pPCA_pen_1 <- pPPCA(lambda=lambda_Deng,Tvotes = 1000,verbose = FALSE,penalty = 1,tau = 0.001,beta = NULL )
    ...
    d_ef_pPCA_pen_1<- tryCatch({
      pPPCA(lambda=lambda_Deng,Tvotes = 1000,verbose = FALSE,penalty = 1,tau = 0.001,beta = NULL )
    ...
    d_ef_pPCA_pen_2<- tryCatch({
     
      pPPCA(lambda=lambda_Deng, Tvotes = 1000,verbose = FALSE,penalty = 2,tau = 0.001,beta = NULL  )
    ...
    d_ef_pPCA_pen_3<-  tryCatch({
      pPPCA(lambda=lambda_Deng,Tvotes = 1000,verbose = FALSE,penalty = 3,tau = 0.001,beta = NULL)
      }, error = function(e) {
      "error" # Devuelve "error" si hay un fallo
    })
    ...
    d_ef_minka_1 <- tryCatch({
      minka2001(lambda=lambda_est, M=nsamp, verbose=FALSE, tau = 0.001, BIC = FALSE)
    }, error = function(e) {
      "error" # Devuelve "error" si hay un fallo
    })
    ...
    d_ef_minka_2 <-  tryCatch({
      minka2001(lambda=lambda_est, M=nsamp, verbose=FALSE, tau = 0.001, BIC = TRUE)
      }, error = function(e) {
      "error" # Devuelve "error" si hay un fallo
    })
    ...
    d_ef_bayesiano <-tryCatch({
      mlcc.bic(xx, numb.clusters = 1, numb.runs = 30, stop.criterion = 1,
               max.iter = 30, max.dim = p, scale = FALSE, numb.cores = NULL,
               greedy = TRUE, estimate.dimensions = TRUE, verbose = FALSE,
               flat.prior = FALSE, show.warnings = FALSE)$subspacesDimensions[[1]]
    }, error = function(e) {
